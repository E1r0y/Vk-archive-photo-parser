...
## Запуск
1. В папку `input` переместить **одну** папку формата `xxxxxxxx` с файлами `messagesXXX.html`.
2. Запустить `main.py`.

Если в `input` нет папок, папок больше одной, или в HTML нет ссылок на фото, скрипт покажет понятное сообщение и завершится без падения.
